...
f.each do |line|
  ...
  if line.downcase=~/validates_presence_of./
    tempfile << "  has_and_belongs_to_many :#{$related_model.downcase}s\n"
  end
end
f.close
tempfile.close
...
f2.each do |line|
  tempfile2<<line
  if line.downcase=~/validates_presence_of./
    tempfile2 << "  has_and_belongs_to_many :#{$new_model_name.downcase}s\n"
  end
end
f2.close
tempfile2.close
...
$habtm_table= $related_model.downcase < $new_model_name ? "#{$related_model.downcase}s_#{$new_model_name.downcase}s" : "#{$new_model_name.downcase}s_#{$related_model.downcase}s"
$migrate_file=%x[ls db/migrate/*create_#{$new_model_name.downcase}s.rb].chomp
...
f3.each do |line|
  ...
  if nextline
    ...
    if $related_model.downcase < $new_model_name
      tempfile3 << "      t.belongs_to :#{$related_model.downcase}s\n"
      tempfile3 << "      t.belongs_to :#{$new_model_name.downcase}s\n"
    else
      tempfile3 << "      t.belongs_to :#{$new_model_name.downcase}s\n"
      tempfile3 << "      t.belongs_to :#{$related_model.downcase}s\n"
    end
...
puts "DO NOT FORGET to edit app/views/#{$related_model.downcase}s/list.html.erb"
